...
**Type:** primary paper
**Role in the deep research:** Fetched under the "Reporting checklists & guidelines" search angle, with 5 claims extracted at the fetch stage. HONESTY NOTE: none of those claims survived into the final verified findings of the synthesis. Its role for Bayesify is supplementary corroboration of Bayesian reporting-guideline practice in biostatistics, not a grounding source for any verified finding.
...
- Scope caveat: BASIS targets simulation studies evaluating statistical methods, not applied Bayesian data analyses — the primary object Bayesify scores — so its checklist transfers only partially.
