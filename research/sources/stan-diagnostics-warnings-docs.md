...
- Corroborates the S4 (computation/diagnostics) catalog in rubric/steps.yaml: the set of diagnostics VeriBayes checks for (divergences, max treedepth, E-BFMI, R-hat, Bulk/Tail ESS) matches what Stan itself flags.
...
- Serves as the phrasing model for VeriBayes deterministic detectors: findings should mirror the warning text and remediation framing users actually see from Stan.
