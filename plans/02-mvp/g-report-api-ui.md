...
**v0 items covered:** D1 layered views · D2 prioritized suggestions · D3 specific praise · A5 override scaffolding · F3 privacy/UI surfaces · C6 cost & budget surfaces
...
## Purpose
The transport and presentation layer: FastAPI job loop, SSE progress, persistence, overrides, and the React SPA that turns a `ScoredResult` into the four-audience report. It owns the shared contracts (`schema.py`) and the stub engine, so the app is end-to-end demoable from M1 and every component a–f can land additively behind it.
...
```
┌ Header ────────────────────────────────────────────────────────────┐
│ BADGE: Verified | Shaky | Failed  + score profile + what_if[]      │
...
**Four layered views (D1, one report, four audiences):** **Author** — prioritized fix-list (linter severities) with concrete `how_to` + brms/Stan/PyMC exemplars. **Reviewer** — exportable, evidence-cited critique block. **Student** — each step links to its methodological source (`standards[]`) + a "what good looks like" example. **Meta-research** — the raw `ScoredResult` JSON, byte-identical to what Phase 3 stores.
...
**Edge semantics:** `Relevance.label = no` short-circuit → report renders the gate page (label, confidence, rationale, evidence_refs, searched-inventory, explicit "not graded" statement, and the **"Run full assessment anyway"** escape hatch wired to the rerun endpoint) — never a badge or forced score; `partial` renders the "limited Bayesian content" banner above a normal report. `StepAssessment.applicable = false` steps render collapsed under "Not applicable" with `applicability_reason`. Local mode renders from `ParsedDoc` + `Evidence[]` only. The meta-research view must round-trip: `parse(report.json) == ScoredResult` exactly.
...
- **Snapshot tests per view (M6):** author / reviewer / student / meta-research rendered from recorded `ScoredResult` fixtures — one Verified empirical, one Shaky with overrides, one relevance-`no` short-circuit, one local-mode inventory. D2 ordering (severity→weight→ease) unit-tested on a shuffled fixture; D3 lint test proves an uncited `did_well[]` entry is flagged.
...
**M6:**
- [ ] Four views render from `ScoredResult` with snapshot coverage; `report.json`/`.md` exports; meta-research JSON round-trips exactly.
- [ ] D2 ordering rule implemented and tested; D3 uncited-praise lint in place; what-if explainer and evidence-span PDF highlighting render.
...
- **Public dashboards / opt-in badges** — Phase 3 (improvements §E2).
